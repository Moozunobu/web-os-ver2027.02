...
import { OS_VERSIONS, findOSVersion } from '../../config/osVersions';

export const TerminalApp: React.FC = () => {
  const [history, setHistory] = useState<string[]>([
    'Abord OS [Version 1.0.0]',
    '(c) Abord Corporation. All rights reserved.',
    '',
  ...
  const [osPromptState, setOsPromptState] = useState<boolean>(false);
...
<p style="font-family: sans-serif; font-size: 12px; color: #666666; margin-top: 30px; text-align: right; border-top: 1px dashed #dddddd; padding-top: 10px;">
  ...
  // Helper to execute OS redirect
  const executeOsRedirect = (targetUrl: string, label: string) => {
    setTimeout(() => {
      try {
      ...
      switch (command) {
        case 'help':
          newOutput.push(
            ...
            '  ver / version             - Display Web OS version information',
            '  os / select-os            - Select and switch Web OS version',
            ...
            '  aboad <url>               - Change or view Aboad Web App Git URL',
            '  git remote set-url <url>  - Set Aboad Git repository URL',
        ...
        case 'aboad-url':
        case 'set-aboad-url': {
          const currentAboadUrl = localStorage.getItem('aboad_git_url') || 'https://moozunobu.github.io/Abord-browser-ver2/';
          if (!args) {
            newOutput.push(
              'Aboad Web App Git Configuration:',
              `  Current URL : ${currentAboadUrl}`,
              '',
              'Usage:',
              '  aboad <url>            - Set Aboad App URL',
              '  aboad set <url>        - Set Aboad App URL',
              '  aboad git <url>        - Set Aboad App Git repository URL',
              '  aboad reset            - Reset Aboad App URL to default'
            );
          } else if (args.toLowerCase().trim() === 'reset') {
            localStorage.removeItem('aboad_git_url');
            window.dispatchEvent(new Event('aboad_url_updated'));
            newOutput.push(
              'Aboad App URL reset to default:',
              '  URL: https://moozunobu.github.io/Abord-browser-ver2/'
            );
          } else {
            let target = args.trim();
            if (target.toLowerCase().startsWith('set ')) target = target.slice(4).trim();
            else if (target.toLowerCase().startsWith('git ')) target = target.slice(4).trim();

            if (!target.startsWith('http://') && !target.startsWith('https://')) {
              target = 'https://' + target;
            }

            localStorage.setItem('aboad_git_url', target);
            window.dispatchEvent(new Event('aboad_url_updated'));

            newOutput.push(
              'Successfully updated Aboad Web App URL!',
              `  New URL: ${target}`,
              'Aboad window updated automatically.'
            );
          }
          break;
        }
        ...
        case 'git': {
          ...
          const subCmd = argTokens[0]?.toLowerCase();

          const setGitUrl = (rawUrl: string) => {
          ...
          if (!args) {
            newOutput.push(
              ...
              'usage: git remote set-url origin <url> | git clone <url>'
            );
          } else if (subCmd === 'remote') {
            if (args.includes('-v')) {
              newOutput.push(
                `origin  ${currentAboadUrl} (fetch)`,
                `origin  ${currentAboadUrl} (push)`
              );
            } else if (args.toLowerCase().includes('set-url')) {
              const urlMatch = argTokens[argTokens.length - 1];
              if (urlMatch && urlMatch.toLowerCase() !== 'set-url' && urlMatch.toLowerCase() !== 'origin') {
                const updated = setGitUrl(urlMatch);
                newOutput.push(
                  `Updated remote 'origin' URL to: ${updated}`,
                  'Aboad Web App target re-linked successfully.'
                );
              } else {
                newOutput.push('Usage: git remote set-url origin <url>');
              }
            } else {
              newOutput.push(`origin  ${currentAboadUrl}`);
            }
          } else if (subCmd === 'clone') {
            const cloneUrl = argTokens[1];
            if (cloneUrl) {
              const updated = setGitUrl(cloneUrl);
              newOutput.push(
                `Cloning into 'Aboad'...`,
                `remote: Enumerating objects: 100%, done.`,
                `remote: Total 142 (delta 0), reused 0 (delta 0)`,
                `Receiving objects: 100% (142/142), done.`,
                `Aboad repository set to: ${updated}`
              );
            } else {
              newOutput.push('Usage: git clone <url>');
            }
          } else if (subCmd === 'config') {
            const configUrl = argTokens[argTokens.length - 1];
            if (configUrl && configUrl.toLowerCase() !== 'config') {
              const updated = setGitUrl(configUrl);
              newOutput.push(`git config aboad.url set to: ${updated}`);
            } else {
              newOutput.push(`aboad.url=${currentAboadUrl}`);
            }
          } else {
            newOutput.push(`git: '${subCmd}' is not a recognized git command.`, 'Try: git remote set-url origin <url>');
          }
          break;
        }
        ...
        case 'winver': {
          newOutput.push(
            'Abord2027.2 Web OS [Version 2027.01.29]',
            'Copyright (C) 2027 Abord Corporation. All rights reserved.',
            ''
          );
          break;
        }
        ...
        case 'os_version':
        case 'os-version': {
          const found = args ? findOSVersion(args) : undefined;
          if (found) {
            newOutput.push(
              `Selected ${found.name}.`,
              `Redirecting to ${found.url} ...`
            );
            executeOsRedirect(found.url, found.name);
          } else {
            OS_VERSIONS.forEach((os, idx) => {
              newOutput.push(`${idx + 1}. ${os.name}`);
            });
            setOsPromptState(true);
          }
          break;
        }
        ...
        default: {
          const directMatch = findOSVersion(command);
          if (directMatch) {
            newOutput.push(
              `Selected ${directMatch.name}.`,
              `Redirecting to ${directMatch.url} ...`
            );
            executeOsRedirect(directMatch.url, directMatch.name);
          } else {
  ...
  const handleSubmit = (e?: React.FormEvent) => {
    ...
    if (osPromptState) {
      const match = findOSVersion(inputValue);
      setOsPromptState(false);

      if (match) {
        setHistory(prev => [
          ...prev,
          `select os version : ${inputValue}`,
          `Selected ${match.name}. Redirecting to ${match.url} ...`,
          ''
        ]);
        executeOsRedirect(match.url, match.name);
      } else {
        setHistory(prev => [
          ...prev,
          `select os version : ${inputValue}`,
          'Invalid selection. Cancelled OS version selection.',
          ''
        ]);
      }
      setInputValue('');
