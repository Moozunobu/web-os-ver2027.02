...
import { Edit2, Trash2, Folder, FolderPlus, MoveRight } from 'lucide-react';
  ...
  return (
    <>
      <div
        ...
        title={isJa ? `ダブルクリックで開く: ${title}` : `Double click to open ${title}`}
        ...
        >
          ...
          {type === 'app' && onTogglePin && (
            ...
            >
              ...
              <span>{isPinned ? (isJa ? 'ピン留めを外す' : 'Unpin from taskbar') : (isJa ? 'タスクバーにピン留めする' : 'Pin to taskbar')}</span>
            </button>
          )}
          ...
          {type !== 'folder' && type !== 'recycle' && folders.length > 0 && onMoveToFolder && (
            <div className="border-t border-gray-100 my-1 pt-1">
              <div className="px-3.5 py-1 text-[10px] font-bold text-gray-400 tracking-wider">
                {isJa ? 'フォルダへ移動' : 'Move to Folder'}
              </div>
              {folders.map((f) => (
                <button
                  key={f.id}
                  ...
                  className="w-full text-left px-3.5 py-1.5 hover:bg-slate-100 flex items-center justify-between transition-colors text-gray-700"
                >
                  <span className="flex items-center gap-2 truncate">
                    <Folder className="w-3.5 h-3.5 text-amber-500 shrink-0" />
                    <span className="truncate">{f.title}</span>
                  </span>
                  <MoveRight className="w-3 h-3 text-gray-400 shrink-0" />
                </button>
              ))}
            </div>
          )}
