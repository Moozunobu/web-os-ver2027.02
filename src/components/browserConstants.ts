  ...
  <script>
    (function() {
      ...
      const defaultRoomsList = [
        { id: 'room-general', name: '全般グループ', type: 'group' },
        { id: 'room-random', name: '雑談トーク', type: 'group' },
        { id: 'room-announcements', name: 'お知らせ', type: 'group' }
      ];
      ...
      function getLocalDB() {
        ...
        const defaultDB = { rooms: defaultRoomsList, messages: {} };
        ...
        try {
          ...
          if (endpoint.startsWith('/')) {
            const getSafeUrl = (path) => {
              ...
              if (!baseOrigin) {
                try {
                  if (localStorage.getItem('webos_app_origin')) {
                    baseOrigin = localStorage.getItem('webos_app_origin');
                  }
                } catch(e) {}
              }
              if (!baseOrigin) {
      ...
      async function loadCachedUsers() {
        ...
        try {
          const res = await withTimeout(supabaseClient.from('wetalks_users').select('*'), 1500);
          const data = res.data;
          ...
          if (data && !error && data.length > 0) {
            ...
            if (error && (error.message && (error.message.includes("Could not find the table") || error.code === '42P01' || error.message.includes("schema cache")))) {
      ...
      async function fetchUserIpAndInitName() {
        fetch("https://api.ipify.org?format=json")
          .then(res => res.json())
          .then(data => { if (data && data.ip) currentIp = data.ip; })
          .catch(e => {});
        
        await loadCachedUsers();
        
        const loggedIn = localStorage.getItem('wetalks_logged_in_user');
        const defaultUser = loggedIn || "ゲスト";
        localStorage.setItem('wetalks_logged_in_user', defaultUser);
        sessionStorage.setItem(SESSION_USER_KEY, defaultUser);
        switchView('lobby');
      }
      ...
      async function initSupabase() {
        ...
        if (typeof supabase === 'undefined' && window.supabase === undefined) {
          for (let i = 0; i < 30; i++) {
            if (typeof supabase !== 'undefined' || window.supabase !== undefined) break;
      ...
      function initGlobalPresence() {
        if (!supabaseClient) {
          const count = (cachedUsers && cachedUsers.length > 0) ? cachedUsers.length : 1;
          if (DOM.lobbyOnlineCountTag) {
      ...
      async function fetchAllRooms() {
        ...
        // 1. Local DB rooms
        try {
          const localRooms = (getLocalDB() && getLocalDB().rooms) ? getLocalDB().rooms : [];
        ...
        try {
          ...
          if (apiRooms && Array.isArray(apiRooms)) {
            apiRooms.forEach(r => {
              if (r && r.id) roomMap.set(r.id, r);
            });
          }
        ...
        if (supabaseClient) {
          try {
            const res = await withTimeout(supabaseClient.from('rooms').select('*'), 3000);
            ...
            console.warn("Supabase fetch rooms timeout/fail:", e);
          }
        }
      ...
      async function deleteRoomAndMessages(roomId) {
        if (supabaseClient && !hasSupabaseRoomsTableError && !hasSupabaseMessagesTableError) {
          try {
            await withTimeout(supabaseClient.from('messages').delete().eq('room_id', roomId), 1500);
            await withTimeout(supabaseClient.from('rooms').delete().eq('id', roomId), 1500);
      ...
      async function loadRoomTitleAndRender() {
        ...
        if (DOM.titleText) DOM.titleText.textContent = displayName;
        renderMessages();
      }

      async function renderLobby() {
        if (DOM.lobbyUsernameTag) {
          DOM.lobbyUsernameTag.textContent = getSessionUser();
        }
        
        const rooms = await fetchAllRooms();

        const onlineCount = (cachedUsers && cachedUsers.length > 0) ? cachedUsers.length : 1;
        if (DOM.lobbyOnlineCountTag) {
          DOM.lobbyOnlineCountTag.textContent = "● オンライン: " + onlineCount + "人";
        }
      ...
      async function renderMessages() {
        ...
        try {
          if (supabaseClient && !hasSupabaseMessagesTableError) {
            const res = await withTimeout(supabaseClient.from('messages').select('*').eq('room_id', activeRoomId).order('id', { ascending: true }), 1500);
            const data = res.data;
            const error = res.error;
            if (error) {
              if (error.message && (error.message.includes("Could not find the table") || error.code === '42P01' || error.message.includes("schema cache"))) {
            ...
            if (msgChannel && !hasSupabaseMessagesTableError) {
              try { supabaseClient.removeChannel(msgChannel); } catch(e){}
              msgChannel = null;
            }
            if (!hasSupabaseMessagesTableError) {
              try {
                ...
                msgChannel
                  ...
                  .subscribe(async (status) => {
                    if (status === 'SUBSCRIBED') {
                      try { await msgChannel.track({ user: getSessionUser() }); } catch(e){}
                    }
                  });
              } catch(e) {}
            }
          ...
          try {
            ...
            msgs = getLocalDB().messages[activeRoomId] || [];
          }
        }
      ...
      async function createRoom() {
        ...
        // Always save on shared server API
        try {
          await callServerAPI('/api/wetalks/rooms', 'POST', { id: roomId, name: roomName, type });
        } catch(e) {}

        // Always save in Supabase if client is ready
        if (supabaseClient) {
          try {
            await supabaseClient.from('rooms').insert([{ id: roomId, name: roomName, type }]);
          } catch(e) {
            console.warn("Supabase createRoom error:", e);
          }
        }
